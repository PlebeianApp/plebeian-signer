...
export class WalletComponent extends NavComponent implements OnInit, OnDestroy {
  ...
  checkMigrationNeeded(): void {
    ...
    // If nip60 is enabled and there are proofs in mints, they may be from vault
    // (before relay sync was enabled). Offer migration.
  ...
  async startMigration(): Promise<void> {
    ...
    try {
      ...
      // Ensure wallet privkey exists
      await this.#nip60Service.getWalletPrivkey(identity);

      // Push all proofs to relays
  ...
  private async clearPendingDeposit() {
    await chrome.storage.session.remove('pendingDeposit');
  }

  private async restorePendingDeposit() {
    const data = await chrome.storage.session.get('pendingDeposit');
    const pending = data?.['pendingDeposit'];
  ...
  /**
   * If NIP-60 is enabled, push the updated proofs for a mint to relays.
   * Called after any operation that modifies proofs (receive, send, deposit, nutzap).
   */
  ...
  async redeemNutzaps() {
    ...
    try {
      ...
      if (redeemed.length === 0) {
        ...
        // Push updated proofs for all affected mints
