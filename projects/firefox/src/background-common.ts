...
async function flushDebugLogs(): Promise<void> {
  ...
  try {
    const result = await browser.storage.session.get('extensionLogs');
    const existingLogs: any[] = (result as Record<string, any>)['extensionLogs'] || [];
    ...
    await browser.storage.session.set({ extensionLogs: updatedLogs });
  } catch (error) {
