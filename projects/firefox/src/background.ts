...
// Keep the background alive with a periodic alarm.
// Prevents the background script from being terminated,
// which causes "Extension context invalidated" errors.
const KEEP_ALIVE_ALARM = 'keep-alive';
browser.alarms.create(KEEP_ALIVE_ALARM, { periodInMinutes: 0.4 }); // ~24 seconds

browser.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === KEEP_ALIVE_ALARM) {
    // No-op - the alarm firing keeps the background alive
    return;
  }
});
...
async function tryAutoUnlock(): Promise<boolean> {
  try {
    ...
    const stored = await browser.storage.local.get(PERSISTED_VAULT_KEY_STORAGE);
    const persistedData = (stored as Record<string, any>)[PERSISTED_VAULT_KEY_STORAGE];
    ...
    if (result.success) {
      ...
      await browser.storage.local.remove(PERSISTED_VAULT_KEY_STORAGE);
      return false;
    }
  } catch (error) {
...
// Also try auto-unlock when the background script first loads (handles extension install/update)
tryAutoUnlock();
...
browser.runtime.onMessage.addListener(async (message /*, sender*/) => {
  ...
  if ((message as { type: string })?.type === 'download-json') {
    ...
    browser.downloads.download({ url: dataUrl, filename, saveAs: true });
    return { success: true };
  }
...
async function processNutzapRequest(req: BackgroundRequestMessage): Promise<any> {
  ...
  switch (method) {
    ...
    case 'nutzap.send': {
      ...
      await browser.storage.session.set({ cashuMints: updatedMints } as unknown as Record<string, unknown>);
          ...
          try {
            ...
            if (targetMint) {
              ...
              if (redeemSignerMeta.nip60Enabled && redeemWriteRelays.length > 0) {
                ...
                if (latestSession) {
                  const vaultData = await getBrowserSyncData();
                  if (vaultData) {
                    const encryptedMints = await Promise.all(
