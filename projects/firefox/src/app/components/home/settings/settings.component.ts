...
export class SettingsComponent extends NavComponent implements OnInit {
  ...
  async onToggleStayUnlocked(event: Event) {
    ...
    if (checked) {
      ...
      try {
        ...
        const session = await browser.storage.session.get(null) as Record<string, any>;
      ...
      await browser.storage.local.remove('persistedVaultKey');
    }
  }
  ...
  #downloadJson(jsonString: string, fileName: string) {
    browser.runtime.sendMessage({ type: 'download-json', json: jsonString, filename: fileName });
  }
