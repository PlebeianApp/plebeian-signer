...
// Extract prompt data from the URL
const params = new URLSearchParams(location.search);
const id = params.get('id') as string;

let method: ExtensionMethod = '' as ExtensionMethod;
let host = '';
let nick = '';
let event = '{}';
...
/**
 * Read prompt data from URL parameters.
 */
function loadPromptData(): void {
  method = (params.get('method') ?? '') as ExtensionMethod;
  host = params.get('host') ?? '';
  nick = params.get('nick') ?? '';
  const base64Event = params.get('event') ?? 'e30=';
  try {
    event = Buffer.from(base64Event, 'base64').toString('utf8');
...
/**
 * Populate the prompt UI after loading data from session storage.
 */
function populateUI() {
  let title = '';
  switch (method) {
...
document.addEventListener('DOMContentLoaded', async function () {
  // Load prompt data from URL parameters
  loadPromptData();
  populateUI();
